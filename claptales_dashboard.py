...
# ─── Column mapping ─────────────────────────────────────────────────────────────
COLUMN_MAP = {
    "product": ["title","product name","product title","asin title","name","product"],
    ...
    "rating":  ["reviews rating","rating","star rating","avg rating","average rating","review rating","ratings"],
    "reviews": ["review count","reviews","number of reviews","total reviews","ratings total","number of ratings"],
    "bsr":     ["bsr","best seller rank","sales rank","rank","best sellers rank"],
    "monthly_sales": ["asin sales","monthly sales","estimated monthly sales","est. monthly sales","sales/month","monthly units","parent level sales"],
    "monthly_revenue":["asin revenue","monthly revenue","estimated monthly revenue","est. monthly revenue","revenue/month","monthly sales revenue","parent level revenue"],
...
def detect_col(df, key):
    ...
    for col in df.columns:
        if col.strip().lower() in aliases:
            return col
    # fuzzy fallback: check if any alias is contained within column name
    for col in df.columns:
...
# Fill missing standard cols with NaN
for c in ["product","brand","price","rating","reviews","bsr","monthly_sales","monthly_revenue","category"]:
    if c not in df.columns:
        df[c] = np.nan

# Show helpful debug info if price/rating not detected
missing_key = [c for c in ["price","rating"] if df[c].isna().all()]
if missing_key and uploaded:
    st.error(f"Could not detect these required columns: {', '.join(missing_key)}")
    st.markdown("**Columns found in your file:**")
    st.code(", ".join(raw.columns.tolist()))
    st.markdown("""
**How to fix:** Rename your columns in Excel to match one of these before uploading:

| Required | Accepted column names |
|---|---|
| price | Price, Sale Price, Buy Box Price, Current Price, Selling Price |
| rating | Rating, Star Rating, Avg Rating, Average Rating |
| reviews | Reviews, Number of Reviews, Review Count, Total Reviews |
| brand | Brand, Brand Name, Manufacturer |
| category | Category, Main Category, Department |
| monthly\_sales | Monthly Sales, Estimated Monthly Sales, Est. Monthly Sales |
| monthly\_revenue | Monthly Revenue, Estimated Monthly Revenue |
...
if len(df) == 0:
    st.error("No valid rows found after filtering. Make sure Price and Rating columns contain numeric values.")
    st.stop()
