    ...
    for row_start in range(0, len(top_prods), cols_per_row):
        ...
        for col_idx, (_, prod) in enumerate(top_prods.iloc[row_start:row_start+cols_per_row].iterrows()):
            with cols[col_idx]:
                ...
                img_url  = prod.get("image_url", "") if "image_url" in prod.index else ""
                prod_url = prod.get("url", "") if "url" in prod.index else ""
                title    = str(prod.get("title","Unnamed Product"))[:60] + ("..." if len(str(prod.get("title",""))) > 60 else "")
